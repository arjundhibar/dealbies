...
export default function CouponPage() {
  ...
  const { getCoupon, getCouponBySlug, voteCoupon, updateCouponVote } =
    useData();
  ...
  useEffect(() => {
    const fetchCouponData = async () => {
      setLoading(true);
      try {
        let couponData = await getCouponBySlug(idOrSlug);
        if (!couponData) {
          couponData = await getCoupon(idOrSlug);
        }
        ...
        if (couponData?.postedBy?.id) {
          try {
            const userResponse = await fetch(
              `/api/users/${couponData.postedBy.id}`
            );
            if (userResponse.ok) {
    ...
    fetchCouponData();
  }, [idOrSlug, getCoupon, getCouponBySlug]);
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (couponCardRef.current) {
        ...
        // Show sticky nav only when the coupon card is fully out of view
  ...
  const handleColdClick = async () => {
    ...
    try {
      ...
      let newUserVote: "up" | "down" | undefined = newColdState
        ? "down"
        : undefined;
      ...
      setCoupon((prev) =>
        prev ? { ...prev, score: newScore, userVote: newUserVote } : prev
      );

      // Update global state
      updateCouponVote(coupon.id, newScore, newUserVote);
      ...
      if (coupon) {
        setCoupon((prev) =>
          prev
            ? { ...prev, score: coupon.score, userVote: coupon.userVote }
            : prev
        );
        updateCouponVote(coupon.id, coupon.score, coupon.userVote);
      }
  ...
  const handleCalledClick = async () => {
    ...
    try {
      ...
      const currentUserVote = coupon.userVote;
      let newScore = currentScore;
      let newUserVote: "up" | "down" | undefined = newCalledState
        ? "up"
        : undefined;
  ...
  rounded-md py-4 px-4 sm:p-4 mr-2"
      >
        ...
        <span>
          This coupon will start on{" "}
          {format(startAtDate, "MMMM d, yyyy 'at' HH:mm")}
        </span>
      </div>
    );
  ...
  rounded-md py-4 px-4 sm:p-4 mr-2"
      >
        ...
        <span>
          ...
          {format(expiresAtDate, "MMMM d, yyyy 'at' HH:mm")}
        </span>
      </div>
    );
  }
  ...
  if (isMobile) {
    return (
      <div className={`w-full ${showStickyNav ? "pt-16" : ""}`}>
            ...
            <div className="flex items-center justify-between h-full px-4">
              ...
              <div className="flex items-center bg-[#0f375f0d] rounded-full p-1 dark:bg-dark-tertiary flex-shrink-0">
                ...
                <Button
                  ...
                  className={cn(
                    "rounded-full border h-7 w-7",
                    userVote === "up" || isCalledPressed
                      ? "bg-[#ce1734] text-white border-[#ce1734]"
                      : "border-gray-300"
                  )}
        ...
        <Card
          ref={couponCardRef}
          className="mb-2 overflow-hidden dark:bg-dark-secondary bg-[#fff] px-4 py-2"
        >
          {offerStatus}
          <div className="flex flex-col">
            ...
            <div className="w-full mb-4">
              <div className="w-full h-[300px]">
                <Carousel
                  images={
                    coupon.imageUrls && coupon.imageUrls.length > 0
                      ? coupon.imageUrls.map((img) =>
            ...
            <div className="flex-1">
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    className=" hover:text-dealhunter-redHover text-[#6b6d70] dark:text-[#c5c7ca] font-medium"
                  >
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={handleShare}
                    className="gap-1 hover:text-dealhunter-redHover text-[#6b6d70] dark:text-[#c5c7ca] font-medium"
                  >
              ...
              <p className="text-muted-foreground mb-6 text-base">
                Available at{" "}
                <span className="text-black text-base dark:text-white font-medium">
                  {merchant}
                </span>
              </p>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  size="lg"
                  className="flex-1 h-9 flex items-center justify-center bg-[var(--background-default)] hover:bg-[var(--background-hover)] rounded-full text-base"
                  asChild
                >
                  <a href={couponUrl} target="_blank" rel="noopener noreferrer">
                    Get Deal
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-4">
              <span className="text-lg font-semibold">
                Your vote helps us show you the best coupons. What do you think?
              </span>
            </div>
            <div className="flex gap-2">
          ...
          <div className="p-6">
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <p className="text-xs">
                  Posted by{" "}
                  <span className="text-sm font-bold"> {postedBy.name} </span>
                </p>
                  ...
                  {posterLoading ? (
                    ...
                    <>
                      <span className="flex items-center gap-1 ">
                        ...
                        {new Date(posterUser.createdAt).getFullYear()}
                      </span>
                      <span className="flex items-center gap-1">
                        <Tag className="h-4 w-4" />{" "}
                        {posterUser.dealsPosted || 0}
                      </span>
                      <span className="flex items-center gap-1">
                        <ThumbsUp className="h-4 w-4" />{" "}
                        {posterUser.votesGiven || 0}
                      </span>
                    </>
                  ) : (
                    <>
            ...
            <div className="mt-6 pt-4 border-t text-xs text-muted-foreground">
              <p className="mt-2">
                If you click on a link and/or order something, dealhunter may
                receive a payment from the seller. This has no influence on the
                decision to place an offer or not. For more information, see the
                FAQ and About Us page.
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card className="rounded-t-none border-t-0 bg-[#f3f5f7] dark:bg-[#28292a] text-white mb-2 -mt-1">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-2 px-4 py-2 w-full">
            <Button
              variant="ghost"
              size="sm"
              className="gap-2 text-[#6b6d70] dark:text-[#c5c7ca] hover:text-dealhunter-redHover dark:hover:text-[#f97936] text-sm font-medium"
            >
              <MessageSquare className="" size={20} strokeWidth={2.5} />{" "}
            ...
            <Button
              ...
              className="gap-2 text-[#6b6d70] dark:text-[#c5c7ca] hover:text-dealhunter-redHover dark:hover:text-[#f97936] text-sm font-medium"
            >
              <Hourglass className="" size={20} strokeWidth={2.5} />{" "}
              <span>Expired?</span>
            </Button>
            <Button
              variant="ghost"
              ...
              className="gap-2 text-[#6b6d70] dark:text-[#c5c7ca] hover:text-dealhunter-redHover dark:hover:text-[#f97936] text-sm font-medium"
            >
              <Flag className="" size={20} strokeWidth={2.5} />{" "}
              <span>Report</span>
            </Button>
          </div>
        </Card>
  ...
  return (
    <div
      className={`w-full px-4 lg:w-[1000px] lg:px-0 mx-auto ${
        showStickyNav ? "pt-16" : ""
      }`}
    >
      {/* Sticky Navigation */}
      ...
      <Card
        ref={couponCardRef}
        className="mb-2 overflow-hidden dark:bg-dark-secondary bg-[#fff] pt-[1.5em] pl-[1.5rem] pr-[1.5rem] pb-[1.5rem]"
      >
        {offerStatus}
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="lg:w-[55%] p-6">
            ...
            <div className="flex gap-4">
              ...
              <Button
                size="lg"
                className="flex-1 h-14 flex items-center justify-center bg-[var(--background-default)] hover:bg-[var(--background-hover)] rounded-full text-lg"
                asChild
              >
                <a href={couponUrl} target="_blank" rel="noopener noreferrer">
                  Get Deal
      ...
      <Card className="rounded-t-none border-t-0 bg-[#f3f5f7] dark:bg-[#28292a] text-white mb-2 -mt-1">
        <div className="flex items-center px-6 py-2 w-full">
          <Button
            variant="ghost"
            size="sm"
            className="gap-2 text-[#6b6d70] dark:text-[#c5c7ca] hover:text-dealhunter-redHover dark:hover:text-[#f97936] text-sm font-semibold"
          >
            <MessageSquare className="" size={20} strokeWidth={3.5} />{" "}
            <span>New response</span>
          </Button>
          <Button
            variant="ghost"
            ...
            className="gap-2 text-[#6b6d70] dark:text-[#c5c7ca] hover:text-dealhunter-redHover dark:hover:text-[#f97936] text-sm font-semibold"
          >
            <Hourglass className="" size={20} strokeWidth={3.5} />{" "}
            <span>Expired?</span>
          </Button>
          <Button
            variant="ghost"
            ...
            className="gap-2 text-[#6b6d70] dark:text-[#c5c7ca] hover:text-dealhunter-redHover dark:hover:text-[#f97936] text-sm font-semibold"
          >
            <Flag className="" size={20} strokeWidth={3.5} />{" "}
            <span>Report</span>
          </Button>
        </div>
      </Card>
