    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage all coupons on the platform.</p>
        </div>
        <Button asChild>
      ...
      <div className="flex flex-wrap gap-2">
        <Link href="/admin/coupons">
          <Badge variant={!searchParams.merchant ? "default" : "outline"} className="cursor-pointer">
            All
          </Badge>
        </Link>
        {merchants.map((m) => (
          <Link key={m.merchant} href={`/admin/coupons?merchant=${m.merchant}`}>
            <Badge variant={searchParams.merchant === m.merchant ? "default" : "outline"} className="cursor-pointer">
              ...
              return (
                <TableRow key={coupon.id}>
                  ...
                  <TableCell className="font-medium">
                    <Link href={`/coupon/${coupon.id}`} className="hover:underline" target="_blank">
                      {coupon.title}
                    </Link>
                  </TableCell>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
            <Link
              key={pageNum}
              href={`/admin/coupons?page=${pageNum}${searchParams.merchant ? `&merchant=${searchParams.merchant}` : ""}`}
            >
              <Button variant={pageNum === page ? "default" : "outline"} size="sm" className="h-8 w-8 p-0">
                {pageNum}
              </Button>
            </Link>
          ))}
