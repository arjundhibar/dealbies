    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage all deals on the platform.</p>
        </div>
        <Button asChild>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {categories.map((cat) => (
          <Link key={cat.category} href={`/admin/deals?category=${cat.category}`}>
            <Badge variant={searchParams.category === cat.category ? "default" : "outline"} className="cursor-pointer">
            ...
            {deals.map((deal) => (
              <TableRow key={deal.id}>
                <TableCell className="font-medium">
                  <Link href={`/deal/${deal.id}`} className="hover:underline" target="_blank">
                    {deal.title}
                  </Link>
                </TableCell>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
            <Link
              key={pageNum}
              href={`/admin/deals?page=${pageNum}${searchParams.category ? `&category=${searchParams.category}` : ""}`}
            >
